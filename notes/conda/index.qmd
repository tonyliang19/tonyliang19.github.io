...
2. `conda install -y PKG_NAME` or `pip install PKG_NAME`, install the packages you need depending on where they are supported (**NOTE**: It supports most python packages, but not quite for R packages). If you like to use more R packages, I recommend [Docker](docker.qmd) or renv.